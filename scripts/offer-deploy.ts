...
async function cleanupAdSetDirectory(): Promise<void> {
  return;
}
...
async function parseArguments(): Promise<DeployOptions> {
  ...
  if (!options.json) {
    ...
    console.log(
      'Usage: npx tsx scripts/offer-deploy.ts --json ./scripts/meta/campaigns/sample.json [--md ./scripts/campaigns/campaign.md] [--template "./scripts/meta/DTC Rancher_Value Ladder_Meta Ads Template.csv"]',
    );
    process.exit(1);
  }
...
function toOfferConfig(unified: UnifiedConfig): OfferConfig {
  // If offerConfig exists, use it directly (it has the full marketing config and pricing)
  if ((unified as any).offerConfig) {
    return (unified as any).offerConfig as OfferConfig;
  }

  // Otherwise, build from unified config structure
  console.log('pricing is', unified.shopify.price);
  console.log('campaign name is', unified.campaign.name);
  console.log('product handle is', unified.shopify.productHandle);
  return {
    handle:
      unified.shopify.productHandle ||
      unified.campaign.name.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
    ...
    targetAudience: unified.adsets?.[0]?.audience
      ? JSON.stringify(unified.adsets[0].audience)
      : undefined,
    highlights: unified.shopify.images?.length
      ? ['Includes curated selection', 'Ships fast']
      : undefined,
    marketingConfig: {
      hero: {
  ...
  try {
    ...
    try {
      ...
      const rootTemplate = path.resolve(
        path.join('scripts', 'DTC Rancher_Value Ladder_Meta Ads Template.csv'),
      );
      const metaTemplate = path.resolve(
        path.join(
          'scripts',
          'meta',
          'DTC Rancher_Value Ladder_Meta Ads Template.csv',
        ),
      );
      ...
      const header = rawHeader.filter(
        (h) => !irrelevantColumns.includes(h as never),
      );
